...
%\thanks{University of Helsinki,Finland}
...
as the effect of monovalent ions (except Li$^+$)  on the phase transition temperature is small
(compared to the effect of multivalent ions), it was initially interpreted %by Cevc
...
(see also discussion in Supplementary Information). 
...
% SAMULI: Almost all the points are within the error excepts. One could say that CHARMM is maybe 
...
{\it Finally, it is questionable if the resolution of AFM experiments~\cite{manyes05,manyes06,fukuma07,ferber11,morata12} alone is sufficient to measure ion locations in fluid--like lipid bilayer systems. \todo{This feels like a detached comment\ldots Could we back this claim up, or rephrase? I mean, now it sounds a bit like we came to conclude based on our simulations that the AFM resolution is not enough.}}
...
\todo{J. Melcr: DONE}
The simulation settings were kept consistent with the simulations with NBFIX. The temperature was kept at 310~K with Nos{\'e}--Hoover~\cite{nose84,hoover85} thermostat. 
...
After energy minimization and an equilibration run of 5 ns, 200ns production runs were performed and analysed. In case of the CaCl$_2$ systems 
