...
%Thus, the choline order parameter changes can be used to directly compare the cation binding affinity
...
show stronger Na$^+$ binding affinity (see Fig.~\ref{NAdensities}). Thus we conclude that sodium binding affinity
is overestimated in all the other models than Orange, CHARMM36 and Lipid14. The order parameter changes with NaCl 
in these three models are small (less than 0.02). Thus, with the achieved accuracy we cannot conclude 
...
With this model the $\beta$ order parameter decrease is overestimated but $\alpha$ order parameter
decrease is underestimated. This result is in line with figure~\ref{AvsB} where $\Delta S_{\rm{CH}}^{(\beta)}/\Delta S_{\rm{CH}}^{(\alpha)}$ ratio
is larger in CHARMM36 than in experiments. According to the electrometer concept, the overestimated order parameter
decrease indicates overestimated Ca$^{2+}$ binding. This is the most likely scenario for the models
where changes in both order parameters were overestimated. However, for CHARMM36 with ions by Yoo et al.
...
(mellow slope in bottom of Fig.~\ref{electrometer}). The separation of these possibilities could be done by 
careful comparison of CHARMM36 model to experimental data where the amount of bound charge is known
