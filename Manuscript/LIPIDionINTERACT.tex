...
% definite article omitted in subtitle in text, why have it here?
\title{Molecular electrometer and binding of cations to phospholipid bilayers} 
...
overexagerated structural effects in the headgroups an artificially positively
...
% this part doesn't belong to intro, it belongs to conclustions/results -- and it is already there in other words. (commented out by JM)
...
a PC bilayer interface induce systematic changes in the order parameters of the choline $\beta$ and $\alpha$
segments. Thus, the changes of these order parameters can be used
to determine the binding affinities of the charged objects. The concept is originally
based on the experimentally observed changes in choline order parameters with bound 
cations~\cite{akutsu81,altenbach84}, see Fig. \ref{ordPions} for these quantities as a function
...
is a constant depending on the valency and the position of bound charge, and $i$
...
and increase with bound negative charge. Consequently, the values of $m_i$ are negative for
...
Experimental data in Fig. \ref{ordPions} show only minor changes in the order
...
Figure~\ref{ordPions} also reports the order parameter changes calculated from MD simulations
...
Accordingly, the presence of cations in simulations leads to a decrease 
of the choline order parameters in Fig. \ref{ordPions}, which is in qualitative
...
shown in Fig. \ref{electrometer}. Roughly linear correlation between the bound charge
and the order parameter change is found in all simulation models, which is in line with
the experiments \cite{altenbach84}. However, there are some differences in the 
proportionality constants (i.e. slopes in Fig. \ref{electrometer}) between different
models; especially MacRog and Orange models give relatively steep slopes, whereas CHARMM
...
because of the unambiguous definition of a free/bound ion. 
...
in Supplementary Information. The overestimation of the order parameter response to bound cations (i.e. slopes $m_\beta$ and $m_\alpha$)
...
than in experiments~\cite{akutsu81}; only Lipid14 gives values in agreement with the experimental ratio.
Thus, in all the other tested simulation models $\alpha$ order parameter decrease is underestimated with
...
In this figure, the simulation models are ordered according to the order parameter changes 
(reported in Fig.~\ref{ordPions}), from the smallest to the largest.
...
is observed in all the tested simulation models. 
Consequently, the electrometer concept can be 
used to quantitatively compare the cation binding affinity between experiments and simulations. 
However, we find that in simulations the responses of $\alpha$ and $\beta$ segments
to bound cations do not generally quantitatively agree with the experiments. 
The $\Delta S_{\rm{CH}}^{\beta}/\Delta S_{\rm{CH}}^{\alpha}$ ratio  
agrees with experiments only in Lipid14 model (Fig.~\ref{AvsB}). 
The observed 
overestimations of the order parameter changes with cation concentrations may, in principle, arise
from overbinding of ions or from too sensitive lipid headgroup response on bound cation 
(see also discussion in Supplementary Material). 
...
\begin{tabular}{c c c c c c c c c c c c}
  ...
  Slipid\cite{jambeck12}, AMBER\cite{beglov94,roux96} &   DPPC & 350 (NaCl)  & 128 & 3726 &  57 & 0  &  57 & 323 & 100 & 100 & \cite{slipidsFILESdppc}\todo{J. Melcr: DONE, added trajs to Zenodo}  \\
  Slipid\cite{jambeck12}, AMBER\cite{beglov94,roux96} &   DPPC & 700 (NaCl)  & 128 & 3612 & 114 & 0  & 114 & 323 & 100 & 100 & \cite{slipidsFILESdppc}  \\
  Slipid\cite{jambeck12}, AMBER\cite{beglov94,roux96} &   DPPC & 1000 (NaCl) & 128 & 3514 & 163 & 0  & 163 & 323 & 100 & 100 & \cite{slipidsFILESdppc}  \\
  \hline
...
The best agreement with experiments (i.e., the lowest order parameter changes) is observed for the Orange,
...
Thus we conclude that the sodium binding affinity is overestimated the models other than Orange, CHARMM36 and Lipid14. 
...
Especially interactions between charged molecules and the lipid bilayer might be significantly
affected by the strong Na$^+$ binding which makes the bilayer effectively positively charged.
...
Almost all the tested models overestimate the order parameter decrease as a function of CaCl$_2$ concentration,
as seen in Fig.~\ref{ordPions}. The only exception is CHARMM36 with the recent ion model by Yoo et al.~\cite{yoo16} which is a selected
...
however, in the case of CaCl$_2$ we cannot exclude that the headgroup response is too sensitive for
...
but underestimated for  $\alpha$, in line with Fig.~\ref{AvsB} where $\Delta S_{\rm{CH}}^{\beta}/\Delta S_{\rm{CH}}^{\alpha}$ 
ratio is larger in CHARMM36 than in experiments. 
Since we cannot determine which of the two $\Delta S_{\rm{CH}}^{\beta}$ or $\Delta S_{\rm{CH}}^{\alpha}$
...
ions present in the simulation are bound in the bilayer indicating the strongest binding affinity among the tested
models. The difference is not as clear as in Fig.~\ref{ordPions} because $\alpha$ carbon order parameters 
are the least sensitive to the bound charge in CHARMM36 (Fig.~\ref{electrometer}).
...
significant infuence on binding in Slipids model (see Supplementary Information). Significant reduction of
Ca$^{2+}$ binding was observed with the ion model by Yoo et al~\cite{yoo16}, however, CHARMM36 lipid
model must be further analyzed to fully interpret the results.

Also, the lipid models may have a significant influence on the ion binding behaviour.
% FYI, Use of dashes and hyphens: http://www.dummies.com/how-to/content/how-to-use-long-and-short-dashes-in-your-writing.html 
...
in actual atomistic resolution structures~\cite{botan15}. Hence, the concept allows direct comparison
of Na$^+$ binding affinity between simulations and NMR experiments by using changes in the head group order parameters.
The comparison reveals that most models overestimate the Na$^+$ binding; only Orange, Lipid14, and CHARMM36 
present binding affinity close to realistic. None of the tested models has the required accuracy to interpret
...
needed to make molecular dynamics simulations reliable in physiologically relevant
...
Figure~\ref{OPchangesSCALED} also compares CHARMM36 simulation~\cite{charmmPOPC350mMNaClnoNBFIXfiles} with and without NBFIX~\cite{venable13} for NaCl.
As expected, without NBFIX the order parameter decrease is more significant.
\todo{Discussion will be finished as soon as we have the density profiles. Citations to Zenodo repositories will be added, if available. JM: added, please, check. }
...
\todo{J. Melcr: DONE}
The simulation settings were kept consistent with the simulations with NBFIX. The temperature was kept at 310~K with Nose-Hoover~\cite{nose84,hoover85} thermostat. 
The simulation files are available at~\cite{charmmPOPC350mMNaClnoNBFIXfiles}. 
...
For the simulation at higher NaCl concentrations (350, 700 and 1000~mM), the configuration from  \cite{slipidsFILES} was taken and 
...
%
We used TIP3P water model and ion parameters by Roux~\cite{beglov94,roux96}. 
Stockholm lipids (Slipids) force field~\cite{jambeck12,jambeck12b} parameters were used for phospholipids. 
...
\todo{J. Melcr: done, see above}  % JM: see above
