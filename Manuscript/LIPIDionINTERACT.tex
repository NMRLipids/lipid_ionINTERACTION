...
of CaCl$_2$ or other multivalent ions. [reference to be added?]
...
The effect of monovalent ions (except for Li$^+$)  on phase transition temperature is small, 
...
Finally, changes in bilayer elasticity and area per lipid measured with Atomistic Force Microscopy (AFM)
can be interpreted as Na$ {+}$ binding to phospholipids~\cite{manyes05,manyes06,fukuma07,ferber11,morata12}.
...
In the present work we set out to solve the apparent contradictions between experiments and simulations 
on the binding of cations to phospholipid bilayers. 
To this aim, we use MD simulations to calculate the order parameters of the head group carbons $\alpha$ and $\beta$ 
(see Fig. \ref{POPCstructure}) as a function of cation concentration, and then we compare simulations with experiments. 
We perform simulations with a number of different force fields, 
and focus on the interpretation of NMR data reported as a function of NaCl and CaCl$_2$ concentrations. 
According to the ''electrometer concept'', changes in order parameters of the $\alpha$ and $\beta$ carbons 
...
simulations \cite{ollila15}, therefore the electrometer concept allows the 
comparison of binding affinity between simulations and experiments. 
% I removed "direct" comparison because both the measures and the calculations do not yield binding affinities.
% the comparison is direct for order parameters, not for bonding affinities.
...
However, more recent experiments assigned negative sign to the order parameter of the $\beta$ carbon,
and positive to the $\alpha$ carbon~\cite{hong95a,hong95b,gross97};
...
Figure~\ref{ordPions} shows the head group order parameters values from H$^2$ NMR measures
for DPPC and POPC bilayers~\cite{akutsu81,altenbach84},
with correct signs~\cite{hong95a,hong95b,gross97}, as a function NaCl and CaCl$_2$ concentration. 
Only minor decreases are measured in the presence of NaCl, 
while the effect of CaCl$_2$ is an order of magnitude larger. 
Thus, according to the electrometer concept, 
...
Figure~\ref{ordPions} also reports order parameters calculated from MD simulations
of DPPC and POPC lipid bilayers in the presence of NaCl or CaCl$_2$. 
Details of the simulated systems are reported in Table~\ref{IONsystems} and in Supplementary Information. 
First of all, it should be noted that none of the models used here 
reproduces the order parameters within experimental uncertainty
for pure PC bilayer without ions, indicating structural inaccuracies with varying severity for all models \cite{botan15}.
% hmm, I see a little problem here: the absolute values for the order parameters are not reported here,
...
Second, all simulation models tested here show a decrease of choline order parameters,
hence the structural response of the lipid head groups to the presence of Na$^+$ is qualitatively correct in all simulation models.
Third, the order parameters in Fig.~\ref{ordPions} with added NaCl show 
significantly different behavior for different simulation models, 
and such different behavior is related to differences in ion binding affinities. 
We can clarify the relationship between order parameters and Na$^+$ binding affinities 
by calculating Na$^+$ density profiles from the different simulation models (see Fig.~\ref{NAdensities}). 
...
indicating a strong correlation between Na$^+$ binding affinities and choline order parameters
and therefore supporting the validity of the electrometer concept~\cite{akutsu81,altenbach84,seelig87,scherer89}. 
...
\caption{List of simulations performed in this work. The ion concentrations are calculated as 
   ...
   The lipid force fields are named as in our previous work~\cite{botan15}.
   For ion force fields, specific ion parameters are cited when possible.}\label{IONsystems}
   %
   % it seems to me that it was always possible, as there always is a name - citation when ions are used. Am I wrong?
   % 
...
Let us now examine more in detail the response to the presence of NaCl predicted by the different simulation models tested here.
The best agreement with experiments (i.e., lowest order parameter changes) is observed for the Orange,
CHARMM36 and Lipid14 models (see Figs.~\ref{ordPions}).
These also predict the lowest Na$^+$ densities in the proximity of the membrane (see Fig.~\ref{NAdensities}). 
...
Na$^+$ binding affinity, especially with physiological NaCl concentrations ($\sim$ 150mM) which is relevant for most applications. 
...
of the electrometer concept in molecular simulations. 

We notice that the choline order parameter changes with NaCl are clearly overestimated in all other tested models (see Fig.~\ref{ordPions}), 
related to unrealistically strong Na$^+$ binding affinity to the bilayer (see Fig.~\ref{NAdensities}). 
Such observation raises questions on the quality of the predictions reported in a number of simulation papers... [complete with examples] 
% I know this will piss of a bunch of people, but it seems interesting information to me.
% Also, we already pissed off a much larger number of people with the previous paper... :-)
...
is realistic in the Orange model but overestimated in BergerOPLS model, 
showing that the binding affinity significantly depends on the used lipid parameters. 
...
%
%		I got to this point, 29/1/16, LM
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\todo{There is something wrong in these stoichiometry numbers.}
...
%%%%%%%%%%%%%%%%%%
For statistical error estimate order parameter
...
%%%%%%%%%%%%%%%%%%
% I don't understand the sentence. Can you check if it has the same meaning as the sentence below?
...
The initial configuration contains 72 DPPC lipids and 2880 SPC water molecules.
...
I think these simulations were actually carried out by Samuli, no?
