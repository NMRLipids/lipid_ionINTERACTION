...
cations~\cite{akutsu81,altenbach84}. Further quantification with various positively 
...
and $S_{\rm{CD}} \approx S_{\rm{CH}}$~\cite{ollila16} the relation between 
bound charge per lipid $X^\pm$ and choline order parameters can be written as~\cite{ferreira16}
\begin{equation}\label{electrometer_eq}
S_{\rm{CH}}^{(i)}(X^\pm)=S_{\rm{CH}}^{(i)}(0) + \frac{4}{3}\chi^{-1}m_i X^\pm,
\end{equation}
where the quadrupole coupling constant $\chi$ approximately equal to 167 kHz,
$S_{\rm{CH}}^{(i)}(0)$ is order parameter without bound charges, $m_i$
is constant depending on valency and position of bound charge, and $i$
refers to either $\alpha$ or $\beta$.
In original experiments absolute values for order parameters 
...
\begin{equation}
\Delta S_{\rm CH}^\alpha=-0.16 X^{Ca^{2+}}
\end{equation}
and
\begin{equation}
\Delta S_{\rm CH}^\beta=-0.08 X^{Ca^{2+}},
\end{equation}
where $X^{Ca^{2+}}$ is the amount of bound $Ca^{2+}$ per lipid.
The lines are drawn in Fig.~\ref{electrometer}
\begin{figure}[]
  \centering
