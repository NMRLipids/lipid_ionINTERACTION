...
studies showing that bilayer properties remain unaltered upon the addition of millimolar concentrations of monovalent 
...
The post-2000 'strong binding' view rests on experimental and simulational findings.
At millimolar NaCl concentrations, the rotational and translational dynamics of membrane-embedded fluorescent probes decrease~\cite{bockmann03,vacha09a,harb13}, and Atomic Force Microscopy (AFM) experiments show changes in the bilayer hardness and area per lipid~\cite{manyes05,manyes06,fukuma07,ferber11,morata12};
and in atomistic molecular dynamics (MD) simulations phospholipid bilayers commonly bind Na${^+}$,
...
Some observables have been interpreted in favor of both views.
For example, some simulation studies (e.g. Refs.~\cite{bockmann03}) reported a reduction in lipid lateral diffusion upon Na$^+$ binding,
supporting the interpretation of the reduced lateral diffusion of fluorescent probes~\cite{bockmann03,vacha09a,harb13}
as favoring the post-2000 view;
however, reduction of lipid diffusion was not observed in noninvasive NMR experiments,
suggesting that the fluorescence results arise from Na$^{+}$ interactions with probes rather than with lipids~\cite{filippov09}.
Other simulations (e.g. Refs.~\cite{}) showed a reduction in area per lipid in the presence of NaCl, 
in agreement with AFM experiments~\cite{manyes05,manyes06,fukuma07,ferber11,morata12};
however, the reduction in area was observed at extremely low Na$^+$ concentrations (XXX~mM), 
compared to observations from scattering experiments (YYY~mM)~\cite{pabst07}.
...
but such small effect in calorimetric measurements was also interpreted to indicate that 
Na$^+$ binds too~\cite{bockmann03,klasczyk10}.
Finally, in electrophoresis measurements on phosphatidylcholine (PC) vesicles,
NaCl increases the (initially negative) zeta potential to only about zero,
whereas positive zeta potentials are generally reached with multivalent ions or Li$^+$~\cite{eisenberg79,tatulian87,manyes05,manyes06,klasczyk10}.
...
In the present paper we set out to solve the apparent contradictions
...
in the phospholipid head group (see Fig. \ref{POPCstructure}) can be used to measure the ion affinity to the 
...
we will show that the response of order parameters to penetrating cations
is qualitatively correct in simulations, but that in several MD models the affinity of Na$^{+}$ ions for PC bilayers
is grossly overestimated.
Moreover, we will show that the accuracy of lipid--Ca$^{2+}$ interactions 
