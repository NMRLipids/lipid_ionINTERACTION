...
but the strength of binding depends on the specific model used~\cite{bockmann03,bockmann04,sachs04,berkowitz06,cordomi08,cordomi09,valley11,berkowitz12}. 
...
\begin{figure*}[]
  ...
  \caption{\label{ordPions}
    ...
    Note that the relative large decrease in CHARMM36 with 450~mM CaCl$_2$ arise from more equilibrated binding affinity due to long simulation times.
  }
...
is explicitly known in both, simulations and experiment. The order parameter changes as a function of cationic amphiphiles
...
In principle, this might explain the overestimated order parameter response to CaCl$_2$ with Berger model,
but not overestimated response to NaCl (see discussion in Supplementary Information).
...
(see also discussion in Supplementary Material). Consequently, a careful analysis is 
required when electrometer concept is used to quantitatively compare the cation binding affinity between 
experiments and simulations with current lipid models. Such analysis is performed in the next section.
...
we conclude that the sodium binding affinity is overestimated in all the other models than Orange, CHARMM36 and Lipid14. 
...
however, in the case of CaCl$_2$ we cannot exclude the explanation that the headgroup resonse is too sensitive for
...
is more realistic in CHARMM36, we cannot conclude if Ca$^{2+}$ binding is too strong or weak in this simulation.
This could be resolved by comparing CHARMM36 model to the experimental data with known amount of bound charge 
(e.g. experiments with amphihilic cations~\cite{scherer89}), however, this is beyond the scope of the
current work. 
...
\todo{Stoichiometry discussion is updated.}
...
simulations, but while Na$^+$ ion binding affinity is realistic in the Orange model, it is seriously overestimated 
...
required to equilibrate amount of bound Ca$^{2+}$. The amount bound Calcium
...
long CHARMM36 simulation with 450~mM CaCl$_2$ show relatively lower order parameters due to higher binding
affinity compared to the shorter simulations. The results suggest that in other simulations the binding affinity 
...
other models than in CHARMM36 with Yoo ions.
\begin{figure}[]
  \centering
  \includegraphics[width=8cm]{../scratch/contacts_ca_charmm36.eps} 
  \includegraphics[width=8cm]{../scratch/contacts_ca_slipids.eps} 
...
amphihiles the situation is more straightofoward since all the charges can be assumed 
...
charge per lipid in simulations from Fig.~\ref{electrometer} ($X^+_{\rm max}=0.5\frac{\rm q}{\rm nm^2}\cdot 0.68{\rm nm}^2=0.34\frac{\rm q}{\rm lipid}$) gives maximum
overestimations of $\approx$0.04 and $\approx$0.06 for $\beta$ and $\alpha$ order parameter changes, respectively. In principle,
...
the differences between responses in simulations and experiments are larger than the maximum estimated influence from oversensitivity of 
headgroup. 
